...
class FixMatchCCSSL(Trainer):
    ...
    def compute_loss(self,
        ...
        if self.pseudo_with_ema:
            ...
            if torch.cuda.is_available():
                ...
                logits_x, _ = model(inputs_x_w)
                logits_u_s, _ = model(inputs_u_s)
                logits_u_w, _ = ema_model(inputs_u_w)
                raise ValueError("Unsupported psuedo with ema mode in exp type")
        else:
        ...
        if labels.shape[0] != 0:
            if self.contrast_with_softlabel:  # re-weighting
                select_matrix = None
                ...
                if self.cfg.get("contrast_left_out", False):
                    print("======self.cfg.get(contrast_left_out, False):======")
                    with torch.no_grad():
                        select_matrix = self.contrast_left_out(max_probs)
                    Lcontrast = self.loss_contrast(features,
                                                   labels,
    ...
    def contrast_left_out(self, max_probs):
        ...
        if torch.cuda.is_available():
            ...
            select_elements += torch.eye(contrast_mask.shape[0])
            select_elements[select_elements > 1] = 1
            select_matrix = torch.ones(contrast_mask.shape[0]) * select_elements
